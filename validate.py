...
DEFAULT_SKILLS_ROOT = USER_HOME / ".gemini" / "config" / "skills"
if not DEFAULT_SKILLS_ROOT.exists():
    DEFAULT_SKILLS_ROOT = Path(r"C:\Users\renat\skills")
...
def main():
    if "--repo-check" in sys.argv:
        repo_dir = Path(__file__).parent.resolve() / "skills"
        if not repo_dir.exists():
            print(json.dumps({"error": "skills/ directory not found"}, indent=2))
            sys.exit(1)
        ...
        if not all_valid:
            print(json.dumps({
                ...
                "error": "One or more skills in the repository failed validation rules."
            }, indent=2, ensure_ascii=False))
            sys.exit(1)
        else:
            print(json.dumps({"valid": True, "message": "All repository skills are valid."}, indent=2))
            sys.exit(0)
